...
ar_open (char *name, int t)
{
  ...
  /* Prepend tmp- to the beginning, to avoid file-name crashes after
  ...
  if (!obfd)
      ...
      if (!t)
	{
	  ...
	  if (!ibfd)
	    {
	      printf("%s: Can't open input archive %s\n",program_name, name);
	      maybequit ();
	      return;
	    }
	  ...
	  if (!bfd_check_format(ibfd, bfd_archive))
	    {
	      printf("%s: file %s is not an archive\n",program_name, name);
	      maybequit ();
	      return;
	    }
...
ar_addlib (char *name, struct list *list)
{
  if (obfd == NULL)
    {
      printf("%s: no output archive specified yet\n", program_name);
      maybequit();
    }
  else
    {
...
ar_addmod (struct list *list)
{
  if (!obfd)
    {
      printf ("%s: no open output archive\n", program_name);
      maybequit ();
    }
  else
    {
...
ar_delete (struct list *list)
{
  if (!obfd)
      ...
      while (list)
	{
	  ...
	  if (!found)
	    {
	      printf("%s: can't find module file %s\n",program_name, list->name);
	      maybequit ();
	    }
